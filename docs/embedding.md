...
**A pushed payload is normalized on the way in, so `getPayload()` can differ from the exact string you pushed.** It never differed byte-for-byte anyway — the designer re-serializes, dropping comments and formatting, and already merges `icon.color` into `fill`. On top of that, an incoming payload has its **missing vertical coordinates written out as `0`** (`text`/`multiline` `y`, `line` `y_start`) and any **`multiline` `spacing` key removed**, and the designer shows the user a dismissible info banner naming what changed. This is deliberate: the designer has no document-flow cursor, so it was already *drawing* those elements at `0` — normalizing makes `getPayload()` report what the canvas actually shows instead of a payload that means something different here than on the device. Full rationale in [`docs/spec/odl-gap-report.md`](spec/odl-gap-report.md#import-normalization--what-the-designer-writes-into-an-imported-payload). Re-pushing an already-normalized payload is a no-op, banner included; a payload with explicit coordinates and no `multiline` `spacing` is never altered and never triggers the banner.
...
- **Register an `icon` if the panel can get narrow.** The designer's header collapses its action buttons to icon-only when the row no longer fits (ADR-016) — an action with an icon keeps its meaning as a tooltip, while an action registered with a label alone keeps its text, because a shared placeholder glyph would make two such buttons indistinguishable. A host that registers several label-only actions therefore keeps a wider header floor than one that supplies icons.
