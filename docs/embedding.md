...
- The **orientation control** in the designer chooses the orientation of that
  ...
  the elements, or the exported image.
...
- `rotate` is **not emitted** in the payload today. The effective send-time
  rotation is per-target output metadata — the target's base rotation combined
  with the user's orientation choice — and it will travel through the service
  options seam ([issue #105](https://github.com/schlomo/odl-drawcustom-designer/issues/105)),
  never baked into the design.
