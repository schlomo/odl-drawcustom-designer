...
| `tests/embed/` | Node or jsdom (Vitest) | Embed contract outcomes: pushed displays (`targets`) → canvas/palette/lock, host states → template preview, host actions, mount/destroy/theme behavior (`docs/embedding.md`) |
...
- **Full-app mounts vs 2-core CI runners:** tests that mount the whole app through a real adapter (`tests/embed/standalone-host.test.tsx`) exceed vitest's 5s default test timeout on GitHub's 2-core runners even when they pass locally in seconds — raise it file-level (`vi.setConfig({ testTimeout: 30_000 })`). Synchronize on conditions, never on wall-clock: `waitFor(...)` polling an observable outcome, not fixed sleeps. To reproduce CI-only flakes locally, run the file under CPU contention (a dozen `yes > /dev/null &` processes made a CI-only failure 100% reproducible, PR #95).
- **`waitFor` has its own 1 s budget:** `vi.setConfig({ testTimeout })` does **not** raise it. A raised test timeout hides nothing if the real wait is a 1 s `waitFor` — that mismatch is what made issue #115 look like a lost push. Do not paper over it with a longer `waitFor`; make the outcome deterministic (see next point).
- **Host pushes land in the tick they are made** (issue #115, ADR-017): the shell registers its push target in a *layout* effect, so once the designer's DOM is observable, `act(() => handle.setTargets(...))` applies and renders synchronously — **assert directly, do not poll**. A `waitFor` there is not "safe extra tolerance"; it converts a hard guarantee into a timing race, and its 1 s budget then loses to CI contention. Only a push made *before* the first commit queues, and that queue drains during the commit — pinned by `tests/embed/mount-lifecycle.test.tsx`.
- **Never measure canvas performance in headless Chromium:** headless software-rasterizes canvas — the same drag that costs ~40 ms/move headed measured ~4000 ms/move headless (~100×, all idle raster stalls), which sent the first rounds of the issue #124 investigation chasing a phantom. Perf measurements run **headed** (`headless: false`), on a production build, and report medians across repeated drags.
