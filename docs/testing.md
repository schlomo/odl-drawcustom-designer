...
- **Full-app mounts vs 2-core CI runners:** tests that mount the whole app through a real adapter (`tests/embed/standalone-host.test.tsx`) exceed vitest's 5s default test timeout on GitHub's 2-core runners even when they pass locally in seconds — raise it file-level (`vi.setConfig({ testTimeout: 30_000 })`). Synchronize on conditions, never on wall-clock: `waitFor(...)` polling an observable outcome, not fixed sleeps, and never assert synchronously after `setCapabilities()`/host pushes — the push queue flushes in a React passive effect on a separate macrotask. To reproduce CI-only flakes locally, run the file under CPU contention (a dozen `yes > /dev/null &` processes made a CI-only failure 100% reproducible, PR #95).
