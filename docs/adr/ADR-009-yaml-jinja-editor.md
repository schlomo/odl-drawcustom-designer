...
- **Interaction blocking is immediate, and the contract is "no element mutation while blocked"** — not just the canvas surface. While `yamlBlocked`: `DesignerCanvas` ignores pointer-down and canvas keyboard shortcuts (`blocked` prop); the canvas header's Redo and the multi-selection align/layer toolbar are `disabled` (Undo is the exception — see the replace rule below); the property panel's controls are disabled (`<fieldset disabled>`); the add-element toolbar buttons are `disabled`; and the element list's drag-reorder is off (`draggable={false}` + drop guard; row click still selects). Sidebar display config, simulator mocks, variables, and assets stay active — they do not mutate `elements`. Any NEW element-mutating control must take a `blocked`/`yamlBlocked` prop and disable itself (proper `disabled` attributes, not pointer-event tricks). The property panel shows the blocked message in its empty (no selection) state too.
...
- **Blocking covers controls that EDIT the design, never the ones that REPLACE it** (revised 2026-09-01). The header's **Clear all** and **Load Demo** stay enabled while `yamlBlocked` — they do not read the current design, they overwrite it, so whether it currently parses is irrelevant to them. They were originally in the disabled list above, which trapped the user: the two controls whose entire purpose is resetting out of a bad state were the two a bad state switched off, leaving hand-editing the YAML as the only way back. They remain disabled during a host display preview (issue #109) — a mode the user leaves at will, not a state they can be stuck in — so `HeaderActionToolbar` takes `previewActive`, not the broader `mutationBlocked`. The test for a NEW control is therefore *edit or replace*, not *mutates `elements` or not*.
- **Undo is a replace while blocked, not a history step** (revised 2026-09-01, maintainer: *"it would be nice if a user can undo out of a broken YAML state too"*). There are two undo stacks: CodeMirror owns the *text* history (still live while the editor is focused — the keystroke-level undo, untouched by any of this), and the app owns the *element-model* history. Text that never validated never entered the element history, so app-Undo cannot replay broken keystrokes. While `yamlBlocked` it therefore does something different and says so: `restoreLastValidDesign()` re-commits the current (last-valid) snapshot through the replace seam, consuming **zero** history steps, purely so the editor text is rewritten and the block lifts. It is enabled regardless of `canUndo` — the design it returns to always exists — but never during a display preview (`undoEscapesBlock = yamlBlocked && !displayPreview.active`). Stepping the history back instead would discard the broken text *and* the last valid design, landing the user one design further back than "undo" implies.
- **Redo has no escape counterpart, deliberately.** The unparseable text was never committed, so no forward element state corresponds to it; stepping forward would throw the user's typing away to reach a design they never asked to return to. It stays `disabled` while blocked, with a tooltip saying why rather than looking merely greyed out. Do not add one for symmetry.
- **All three replacing controls confirm while the document is invalid** (ruling 2026-09-01). Clear all, Load Demo and Undo each destroy editor text that nothing has committed — no undo entry, no payload revision, nothing downstream that records it existed — so each asks first. One implementation, `requestReplaceConfirm` in `src/ui/lib/replace-confirm.ts`; do not grow a second dialog path. Scope: **Load Demo** asks whenever there is anything to lose (`shouldConfirmLoadDemo(elementCount, yamlBlocked)` — a broken doc counts, because a frozen `elementCount === 0` does not mean an empty editor); **Clear all** and **Undo** ask *only* while blocked and keep their existing prompt-free behaviour on a valid document. Undo's prompt must state that it returns to the last valid design and name the editor's own Ctrl/Cmd+Z as the keystroke-level alternative — a user expecting character-level undo and losing a paragraph would be right to complain. Its tooltip stays one short line (`ToolbarTooltip` bubbles are `whitespace-nowrap`, and an embedded mount may have no room to either side), so the full detail lives in the prompt.
- **A replacement is exempt from the echo gate, and must say so.** Simply enabling those buttons is not enough: their commit lands in `elements`, but the sync below then refuses to write it into the editor, so the canvas shows the new design while the YAML pane still shows the old broken text and the block never lifts. `YamlPanel` publishes `armExternalYamlReplace` via `armExternalReplaceRef`; `clearElements`, `loadDemo`, `restoreLastValidDesign` and the host payload push call it **synchronously, immediately before** their commit. It discards any parked draft (same reason a host push does) and exempts exactly one sync run. **Arm it only where a commit is guaranteed to follow** — the push applier arms *after* its dedupe early-return for this reason. Correspondingly, **a blocked document disqualifies that dedupe** (Copilot review, PR #180): while blocked the editor and `elements` are out of sync, so the payload a host re-pushes to rescue the user is structurally identical to the frozen `elements` and was silently swallowed — breaking the obvious host resync in the one state that needs it. The condition is now `!hadPendingDraft && !docBlocked && elementsSequenceEqual(...)`, read through a live `docBlockedRef` published by `YamlPanel` (the parent's `yamlBlocked` state lands a render later, and a push can arrive inside that window). A rescue push takes the full apply path — revision bump, history reset, selection reset — deliberately: it observably replaced unparseable text, and an exception would make it a second, subtly different push path. An arm with no commit behind it is left waiting for whatever sync runs next, which would let it clobber an unrelated in-progress edit and reintroduce issue #35.
- **Load Demo's confirmation reads `yamlBlocked` too.** While blocked, `elements` is frozen at last-valid, so `elementCount === 0` does not mean an empty editor — the user may be part-way through hand-typing a design. `shouldConfirmLoadDemo(elementCount, yamlBlocked)` prompts in that case. (Clear all still has no confirmation of its own, unchanged.)
...
**Tests:** `tests/ui/components/yaml-panel-blocked-sync.test.tsx` (real `EditorView` mount: dep toggles never rewrite a broken doc; blocked-state bubbling; type-completion accept unblocks and syncs; typing through a transient invalid state keeps typed text; drag toggle during the debounce window keeps newer text), `tests/ui/hooks/use-yaml-blocked-visibility.test.ts` (grace-period timing, fake timers), `isYamlDocBlocked` cases in `tests/ui/editor/yaml-elements-sync.test.ts`, and the e2e flows `tests/e2e/yaml-blocked-state.spec.ts` (break YAML → overlays appear, canvas clicks inert, edit not reverted → fix YAML → editing resumes) and `tests/e2e/yaml-blocked-escape.spec.ts` (break YAML → Clear all / Load Demo / Undo → design replaced, editor text follows, block lifts, editing resumes; confirmations appear and dismissal is a true no-op; Redo stays disabled), plus `tests/ui/lib/replace-confirm.test.ts` (prompt scope and the Undo wording rules) and the valid-document scope guard in `tests/ui/components/load-demo.test.tsx`.
