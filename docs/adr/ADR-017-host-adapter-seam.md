...
Accepted — implemented in [issue #72](https://github.com/schlomo/odl-drawcustom-designer/issues/72) (maintainer ruling 2026-07-20). Extends
...
`mountStandaloneApp()` ([`src/embed/standalone.tsx`](../../src/embed/standalone.tsx)) and `mount()`
([`src/embed/mount.tsx`](../../src/embed/mount.tsx)) became **sibling** functions over one App shell. Each
...
- `embedded` is a *mode*, so each new host meant another branch inside the
  shell. The M4 HA panel ([issue #25](https://github.com/schlomo/odl-drawcustom-designer/issues/25)) would have been a third mode.
...
([issue #23](https://github.com/schlomo/odl-drawcustom-designer/issues/23)). The **embedded host API surface is unchanged** — `MountOptions`,
...
- **One lifecycle path.** The standalone SPA is the standalone adapter over
  ...
  the standalone wiring, and [`tests/embed/standalone-host.test.tsx`](../../tests/embed/standalone-host.test.tsx) pins the
...
- Document-level theme stays **adapter** code (`applyStoredDocumentTheme` in
  ...
  `document.documentElement`, `document.head` or global theme ([PR #67](https://github.com/schlomo/odl-drawcustom-designer/pull/67)/[#74](https://github.com/schlomo/odl-drawcustom-designer/pull/74)).
...
- The M4 HA panel is an adapter file plus its live-state source; no shell
  changes, no third mode ([issue #25](https://github.com/schlomo/odl-drawcustom-designer/issues/25)).
...
- **Expose the host seam publicly (`mount(container, { host })`)** — rejected:
  ...
  API for one internal consumer ([issue #23](https://github.com/schlomo/odl-drawcustom-designer/issues/23)).
