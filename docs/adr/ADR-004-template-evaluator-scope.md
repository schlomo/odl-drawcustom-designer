...
without changing the answer. An element with **no** template syntax anywhere
AND whose normalization is a no-op is returned **as-is** rather than
deep-copied — nothing to substitute and nothing to normalize means nothing to
copy — which lets consumers that memoize on element identity keep their memo
across an edit elsewhere in the payload. The one template-free exception is an
icon element still carrying the legacy `color` field: normalization migrates
it to `fill` and returns a fresh clone, so that preview does NOT share identity
with the source element. Preview results are read as immutable data and must
never be mutated in place; before this they were always fresh clones, and a
mutating consumer would now corrupt the stored payload — the as-is return is
`Object.freeze`d under `import.meta.env.DEV`/test builds to make an accidental
mutation throw instead of silently corrupting `elements`.
