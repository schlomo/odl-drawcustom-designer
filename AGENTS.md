...
- **Host assets are the LAST resolution tier** (issue #138, ADR-002): local content map → bundled → `resolveAsset`. Never let the host tier pre-empt an uploaded or bundled asset, never teach the designer a search path (the host owns those), and never let an unsupplied asset degrade into a substituted font or a silent gap — it must reach the existing explicit render-error state naming the asset (issue #10).
