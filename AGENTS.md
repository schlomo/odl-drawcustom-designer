...
- **New controls: classify edit vs. replace before gating on document validity** (ADR-009, revised 2026-09-01) — blocking must cover controls that EDIT the design, never ones that REPLACE it (Clear all, Load Demo, Undo-to-last-valid, a host push). Gating everything on "YAML valid" trapped the user with the only way out disabled.
...
- **One channel per concern** (2.0, issue #121 — do not reintroduce a second one):
  ...
  - **One display mapping:** `displaySpecToCanvas` resolves from the designer's canonical defaults (only `previewDitherMode` carries over from the current canvas). Never re-add a merge-onto-current base — it leaks one panel's rotation or measured palette onto another (ADR-007 parity).
...
- Hidden overlays/tooltips must use `display:none`, not `visibility:hidden`/`invisible` — hidden layout boxes widen scrollers (horizontal-scrollbar bug class, PR #85; see issue #86 for remaining instances). The one legitimate `invisible` is an ADR-016 **measurement probe** (`display:none` has no layout to measure), and only inside a `fixed`, `h-0`, `overflow-hidden` wrapper that is out of flow.
- **Every toolbar row is measured, including the page header** (ADR-016): a `shrink-0` row that never collapses sets the document's horizontal floor and, in an embedded mount, scrolls the HOST page. Inside the header, priority is fixed: the action buttons drop their **labels** first, and only then does the meta row drop **whole segments** (privacy → GitHub → branch → SHA → build identity). Metadata that no longer fits is removed, never ellipsed into a stub, and the build identity (`v{version}` / `PR #n`) is the last thing standing. Guarded by `tests/e2e/header-horizontal-overflow.spec.ts` — jsdom does no layout, so a pure-Vitest assertion cannot see this bug class. When verifying a reported layout/overflow bug via browser automation, check `document.visibilityState` first — a backgrounded tab throttles `requestAnimationFrame`/`ResizeObserver` and can report a collapse that does not exist in a visible tab at the same width.
- **A write affordance must land somewhere the eventual consumer can read.** Content Manager uploads went into browser IndexedDB and rendered fine on canvas (read back from that same store), then failed the moment a host resolved assets itself — found on a maintainer's live Home Assistant panel, not hypothesized. `hostOwnsAssets` (ADR-002) turns that path read-only when a host declares it; apply the same read/write-store split to any future host-visible content type.
...
**Verify parity claims against the vendored `odl_renderer` or measured library behaviour — never our own spec doc or reasoning.** `docs/spec/supported_types.md` had multiline's `offset_y` and `spacing` inverted until reading the vendored renderer settled it (issue #169, fixed in PR #178). Library semantics (e.g. Pillow anchor offsets) get MEASURED, not reasoned about — guessing produces a confident, subtly wrong fix.
...
- Only the `version` job needs `fetch-depth: 0` (tags + history); the publish jobs must stay shallow. That job also carries `filter: blob:none` — safe there because it reads only refs/commits and the checked-out tree. **Never copy that filter to the `pages` job**: `JamesIves/github-pages-deploy-action` fetches `gh-pages` inside the same checkout and a promisor clone cannot satisfy it — PR #174 shipped exactly that and silently broke production deploys while all PR checks stayed green (reverted in #176).
- Both publish steps must stay **idempotent** — re-running a failed job is the recovery path. `gh release create` is **not atomic** (tag + release, then assets one by one), so `tools/createRelease.ts`'s `already-released` path **reconciles** the release's assets instead of exiting; never turn it back into an early return. The npm job takes its bytes from the release itself (`gh release download`), never from a workflow artifact — artifact visibility across run *attempts* is unspecified and "re-run the failed npm job alone" is the headline recovery path.
...
- Review-bot comments (e.g. Copilot) may target a stale revision — verify each claim against the current pushed commit before acting; reply-and-resolve with evidence when a finding is already fixed or refuted. Even a wrong mechanism can point near a real hole: a Copilot finding against `withInsertedKey` was unreachable as described, but the adjacent defect was real — a bare `y:` parses to `null`, not "missing", and blocked the canvas (PR #179).
- **Every PR body carries two sections** (maintainer ruling 2026-08-16): **Verified** — what was tested and how (gate results, red-first evidence, review passes, e2e), and **Maintainer validation** — what only the human should check before merging (manual test steps, policy calls baked in, explicitly UNVERIFIED items). Update both when the PR changes. When a change is provable only in production (a CI pipeline shape, a cross-run artifact hand-off), that is a reason to sequence it behind something small, not a footnote.
- **Outward-facing text gets its own check pass.** GitHub PR bodies cap at 65,536 characters — budget before assembling a long one. Verify authorship and "what we added" against the actual diff before crediting a PR or presenting existing upstream tooling as this project's own contribution — both have produced near-misses in an upstream PR body.
- **A "clean" sweep is only as good as its mechanism.** `\b` is not a word boundary in this environment's `grep`/`git grep` — a `\b`-anchored rename or leftover sweep matches nothing and reads as clean; use `-w`. Enumerate types/fields from the authoritative constant, not a convenient grep — `DRAW_ELEMENT_TYPES` (`src/core/schema/elements.ts`) lists 16 element types; grepping insertion templates undercounted at 13 and missed `polygon`/`arc`/`icon_sequence`.
...
- **One agent per worktree, even within a single task's rounds.** The per-task worktree isolates branches; nothing stops two agents sharing one worktree at the same time — a brief containing `git reset --hard` nearly destroyed another agent's uncommitted work there. Serialize rounds in a shared worktree, and check `git status` before dispatching the next one.
- **Write dispatch briefs to invite refutation of your hypothesis, not confirmation of it.** Asking an agent to confirm-or-refute with evidence overturned wrong assumptions repeatedly — treat a strong hunch as a question to the agent, not a given.
- **After an agent dies mid-round (provider overload, etc.), verify its worktree before continuing** — `git status`/`git log`, then restate the brief in full to a fresh agent rather than resuming a dead transcript.
